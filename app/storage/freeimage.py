...
from app.config import FREEIMAGE_API_KEY
...
class FreeImageStorage(StorageProvider):
    ...
    async def upload(
        ...
        if not FREEIMAGE_API_KEY:
            raise Exception("FREEIMAGE_API_KEY chưa được cấu hình")
        ...
        return {
            ...
            "size": image.get("size"),
            "filename": image.get("filename", filename),
        }
    ...
    async def delete(self, file_id: str):
        return {
            ...
            "file_id": file_id,
            "deleted": False,
            "message": "Delete handler sẽ hoàn thiện sau"
        }
